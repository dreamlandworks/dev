...
<div class="content-wrapper">
  ...
  @endif
    ...
    <h3 class=" rv-list-users">BlueCollor Move Booking</h3>
        ...
        <div class="col-md-12">
            <div class="card p-3 m-3-rv">
                <div class="card-header">
                    <a href="{{url('booking-cb/create')}}" class="btn btn-primary">Add +</a>
                   </div>
                <div class="card-body">
                   <table id="km_dt" class="table table-bordered table-hover dataTable no-footer">
                        ...
                        <tbody class="mk_tbodynowrap">
                            @foreach ($cbookings as $cbooking)
                              <tr>
                                <td>{{$cbooking->id}}</td>
                                <td>{{$cbooking->users_id}}</td>
                                <td>{{$cbooking->userdetail ? $cbooking->userdetail->fname : ''}}</td>
                                <td>{{$cbooking->scheduled_date}}</td>
                                <td>{{$cbooking->started_at}}</td>
                                <td>{{$cbooking->completed_at}}</td>
                                <td>{{$cbooking->amount}}</td>
                                <td>{{$cbooking->sgst}}</td>
                                <td>{{$cbooking->cgst}}</td>
                                <td>{{$cbooking->spdetail ? $cbooking->spdetail->fname : ''}}</td>
                                <td>
                                <button id="reschedule_btn"  type="button" class="btn btn-primary btn-sm" onclick="bookingschedule({{$cbooking->id}})">Rescheduled</button>&nbsp;&nbsp;&nbsp;
...
<!--Reschedule booking-->
<div class="modal fade" id="cb_booking_resch" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
  aria-hidden="true">
...
function re_schedule()
{
    $.ajax({
            ...
            data: { 
              ...
              "_token": "{{ csrf_token() }}"},
            success:function(data){
              $('#cb_booking_resch').modal('hide');
              console.log(data);
              if(data=="success") 
              {
                  var msg='<div class="col-md-12">';
                  msg+='<div class="alert alert-success alert-dismissible fade show" role="alert">';
                  msg+='Blue Collor booking has been successfully rescheduled.';
