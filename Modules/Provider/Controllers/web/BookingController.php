...
class BookingController extends ResourceController
{
    ...
    public function single_move_booking()
    {
        if ($this->request->getMethod() != 'post') {
            ...
            if (
                !array_key_exists('scheduled_date', $json) || !array_key_exists('time_slot_from', $json)
                ...
                || !array_key_exists('sgst', $json) || !array_key_exists('cgst', $json)
                ...
                || !array_key_exists('profession_id', $json)
            ) {
                return $this->respond([
                    'status' => 403,
    ...
    public function blue_collar_booking()
    {
        if ($this->request->getMethod() != 'post') {
            ...
            if (
                ...
                || !array_key_exists('sgst', $json) || !array_key_exists('cgst', $json)
                || !array_key_exists('attachments', $json) || !array_key_exists('estimate_time', $json) || !array_key_exists('estimate_type_id', $json)
                || !array_key_exists('users_id', $json) || !array_key_exists('key', $json) || !array_key_exists('profession_id', $json)
    ...
    public function booking_payments()
    {
        if ($this->request->getMethod() != 'post') {
            ...
            if (
                !array_key_exists('booking_id', $json) || !array_key_exists('date', $json) || !array_key_exists('booking_amount', $json)
                || !array_key_exists('reference_id', $json) || !array_key_exists('payment_status', $json) || !array_key_exists('cgst', $json)
                || !array_key_exists('time_slot_from', $json) || !array_key_exists('sp_id', $json) || !array_key_exists('sgst', $json)
                || !array_key_exists('users_id', $json) || !array_key_exists('key', $json) || !array_key_exists('w_amount', $json)
            ) {
                return $this->respond([
                    'status' => 403,
                    'message' => 'Invalid Parameters'
                ...
                if ($key != $api_key) {
                    ...
                    if ($transaction_id > 0) { //Insert into Booking Payments/booking_receipts 
                        if ($json->payment_status == 'Success') {
                            ...
                            $arr_booking_status = array(
                                'booking_id' => $json->booking_id,
                                'status_id' => ($json->payment_status == 'Success') ? 9 : 30,
                                'created_on' => date('Y-m-d H:i:s')
                            );
                            $common->insert_records_dynamically('booking_status', $arr_booking_status);
                            ...
                            $arr_alerts = array(
                                ...
                                'profile_pic' => $sp_profile[0]['profile_pic'],
                                'status' => 2,
                                'created_on' => $date,
                                'updated_on' => $date
                            );
                            ...
                            $arr_alerts1 = array(
                                ...
                                'user_id' => $json->sp_id,
                                'profile_pic' => $user_profile[0]['profile_pic'],
                                'status' => 2,
                                'created_on' => $date,
                                'updated_on' => $date
                            );
            ...
            if (!array_key_exists('booking_id', $json) || !array_key_exists('category_id', $json) || !array_key_exists('users_id', $json) || !array_key_exists('key', $json)) {
                ...
                if ($key == $api_key) {
                    ...
                    if ($arr_booking_details != 'failure') {
                        ...
                        $arr_booking['expenditure_incurred'] = ($arr_booking_details['expenditure_incurred'] != Null) ? intval($arr_booking_details['expenditure_incurred']) : "";
                ...
                if ($key == $api_key) {
                    ...
                    if ($arr_single_move_booking_details != 'failure') {
                        foreach ($arr_single_move_booking_details as $key => $book_data) {
                            ...
                            if ($reschedule != 'failure') {
                                $reschedule_date = $reschedule[0]['rescheduled_date'];
                                $reschedule_time = $reschedule[0]['from'];
                                $reschedule_description = $reschedule[0]['description'];
                            } else {
                                $reschedule_date = "";
                                $reschedule_time = "";
    		               ...
    		               $arr_booking[$key]['zipcode'] = $book_data['zipcode'];*/
                            ...
                            $arr_booking[$key]['reschedule_date'] = ($reschedule_date == null ? "" : $reschedule_date);
                            $arr_booking[$key]['reschedule_time'] = ($reschedule_time == null ? "" : $reschedule_time);
                            $arr_booking[$key]['reschedule_description'] = ($reschedule_description == null ? "" : $reschedule_description);
                            ...
                            $arr_booking[$key]['otp'] = ($book_data['otp'] == Null ? "" : $book_data['otp']);
                            $arr_booking[$key]['extra_demand_total_amount'] = ($book_data['extra_demand_total_amount'] == Null ? "" : $book_data['extra_demand_total_amount']);
                            $arr_booking[$key]['material_advance'] = ($book_data['material_advance'] == Null ? "" : $book_data['material_advance']);
                            $arr_booking[$key]['technician_charges'] = ($book_data['technician_charges'] == Null ? "" : $book_data['technician_charges']);
                            $arr_booking[$key]['expenditure_incurred'] = (($book_data['expenditure_incurred'] == Null)  ? "" : intval($book_data['expenditure_incurred']));
                    ...
                    if ($arr_blue_collar_booking_details != 'failure') {
                        foreach ($arr_blue_collar_booking_details as $bc_book_data) {
                            ...
                            $bc_reschedule = $common->get_details_dynamically('re_schedule', 'reschedule_id', $bc_reschedule_id);
                            if ($bc_reschedule != 'failure') {
                                $bc_reschedule_date = $bc_reschedule[0]['rescheduled_date'];
                                $bc_reschedule_time_id = $bc_reschedule[0]['rescheduled_time_slot_id'];

                                $bc_re_time = $common->get_details_dynamically('time_slot', 'id', $bc_reschedule_time_id);
                                $bc_reschedule_time = $bc_re_time[0]['from'];
                            } else {
                                $bc_reschedule_date = null;
                                $bc_reschedule_time = null;
                            }
                            ...
                            $arr_booking[$booking_count]['reschedule_id'] = $reschedule_id;
                            ...
                            $arr_booking[$booking_count]['req_raised_by'] = ($reschedule_data != 'failure' ? $reschedule_data[0]['req_raised_by_id'] : "");;
                    ...
                    if ($arr_multi_move_booking_details != 'failure') {
                        ...
                        foreach ($arr_multi_move_booking_details as $mm_book_data) {
                            if (!array_key_exists($mm_book_data['id'], $arr_exists)) {
                                ...
                                $mm_reschedule = $common->get_details_dynamically('re_schedule', 'reschedule_id', $mm_reschedule_id);
                                if ($mm_reschedule != 'failure') {
                                    $mm_reschedule_date = $mm_reschedule[0]['rescheduled_date'];
                                    $mm_reschedule_time_id = $mm_reschedule[0]['rescheduled_time_slot_id'];

                                    $mm_re_time = $common->get_details_dynamically('time_slot', 'id', $mm_reschedule_time_id);
                                    $mm_reschedule_time = $mm_re_time[0]['from'];
                                } else {
                                    $mm_reschedule_date = null;
                                    $mm_reschedule_time = null;
                                }
    ...
    public function sp_booking_response()
    {
        if ($this->request->getMethod() != 'post') {
            ...
            if (
                !array_key_exists('booking_id', $json) || !array_key_exists('status_id', $json)  || !array_key_exists('users_id', $json)
                || !array_key_exists('amount', $json) || !array_key_exists('sp_id', $json) || !array_key_exists('created_on', $json)
        ...
        if ($validate_key == "" || $validate_booking_id == "") {
            ...
            if ($key == $api_key) {
                ...
                if ($validate_sp_id > 0) { //Booking Started
                    ...
                    $common->update_records_dynamically('users',['online_status_id' => 2],'id',$sp_id);
    ...
    public function reschedule_booking()
    {
        if ($this->request->getMethod() != 'post') {
            ...
            if (
                !array_key_exists('booking_id', $json)  || !array_key_exists('scheduled_date', $json) || !array_key_exists('scheduled_time_slot_id', $json)
                    ...
                    if ($reschedule_id > 0) {
                        ...
                        if ($json->user_type == 'User') {
                            ...
                            $arr_alerts1 = array(
                                ...
                                'user_id' => $sp_id,
                                'profile_pic' => $sp_details['profile_pic'],
                                'status' => 2,
                                'created_on' => $date,
    ...
    public function update_reschedule_status_by_sp()
    {
        ...
        $json = $this->request->getJSON();
        if (
            !array_key_exists('booking_id', $json) || !array_key_exists('reschedule_id', $json) || !array_key_exists('status_id', $json) || !array_key_exists('user_type', $json)
    ...
    public function cancel_booking()
    {
        ...
        $json = $this->request->getJSON();
        if (
            !array_key_exists('booking_id', $json) || !array_key_exists('status_id', $json) || !array_key_exists('reasons', $json)
                    ...
                    if ($status_id == 24) {
                        ...
                        if ($arr_wallet_details_sp != 'failure') {
                            ...
                            $arr_insert_data = array(
                                'users_id' => $sp_id,
                                'amount' => 0 - ($cancel <= 0 ? 0 : $cancel),
                                'amount_blocked' => 0
                            );

                            $common->insert_records_dynamically('wallet_balance', $arr_insert_data);
                        }
                    }
    ...
    public function complete_booking()
    {
        if ($this->request->getMethod() != 'post') {
            ...
            if (
                !array_key_exists('booking_id', $json) || !array_key_exists('users_id', $json)  || !array_key_exists('sp_id', $json) || !array_key_exists('payment_status', $json)
                || !array_key_exists('completed_at', $json) || !array_key_exists('total_amount', $json) || !array_key_exists('amount', $json) || !array_key_exists('reference_id', $json)
                || !array_key_exists('cgst', $json) || !array_key_exists('sgst', $json) || !array_key_exists('method_id', $json) || !array_key_exists('key', $json) || !array_key_exists('w_amount', $json)
            ) {
                return $this->respond([
                    'status' => 403,
                    'message' => 'Invalid Parameters'
                ...
                if ($key == $api_key) {
                    ...
                    $common->update_records_dynamically('users',['online_status_id' => 1],'id',$json->sp_id);
                    ...
                    if ($booking_data != "failure" && $booking_data[0]['status_id'] != 23) {
                        ...
                        if ($json->payment_status != "failure") {

                            if ($json->amount != 0 && $json->w_amount != 0) {
                                $arr2 = [
                                    [
