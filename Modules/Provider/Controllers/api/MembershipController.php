...
class MembershipController extends ResourceController
{
    ...
    //---------------------------------------------------------TXN Membership Payments for SP-------------------------------------------------
    //-------------------------------------------------------------**************** -----------------------------------------------------
    public function membership_payments_txn_sp()
    {
        if ($this->request->getMethod() != 'post') {
            ...
            if (
                !property_exists($json, 'amount') || !property_exists($json, 'sp_id')
                ...
                if ($key == $api_key) {
                    ...
                    $result = $paytm->gettxn($order_id, $json->amount, $json->sp_id);
            ...
            if (
                ...
                if ($key == $api_key) {
                    ...
                    if ($json->w_amount == 0) {
                        ...
                        $arr_transaction['users_id'] = $json->sp_id;
                    ...
                    if ($transaction_id > 0) { //Insert into sp_subs_plan
                        ...
                        $arr_alerts = array(
                            'type_id' => 4,
                            'description' => "Your new plan not activated due to payment failure ",
                            'user_id' => $json->sp_id,
                            'profile_pic_id' => $json->sp_id,
                            'status' => 2,
                            'created_on' => $json->date,
